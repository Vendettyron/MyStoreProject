...
export async function registerClientService(data: RegisterClientSchemaDTO) {
    ...
    if (authError) throw new AppError("Error al crear al crear tu usuario, vuelve a intentarlo", HttpStatus.INTERNAL_SERVER_ERROR_500);
    ...
    if (spError) throw new AppError("Error al crear al crear tu usuario, vuelve a intentarlo", HttpStatus.INTERNAL_SERVER_ERROR_500);
